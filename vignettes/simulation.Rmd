...
compute_classification_metrics <- function(p, torc=3){
  ...
  metrics_dt <- bind_cols(maxfrac_fp,
    ...
    mutate(cutoff = c("mf_0.60", "mf_0.80", "mf_0.90")) %>%
...
S <-6
rmax <-15
#prob <- runif( S, min=.05, max=.5)
prob <- c(0.5, 0.4, 0.3, 0.2, 0.15, 0.1)
```
...
```{r}
save_plot(file.path(figures_dir,"simulation_tradoff.pdf"), 
          p_tradeoff_all,
